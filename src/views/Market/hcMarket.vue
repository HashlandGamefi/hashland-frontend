  ...
  <div class="busd_page">
    ...
    <div class="info_box">
      <div class="onebox" v-for="(item, index) in infoArr" :key="index">
        ...
        <p class="p1 fontsize22" v-else>{{ item.num }} HC</p>
      </div>
    </div>
    <div class="content">
      <!-- 排序  卡牌信息 -->
...
import { hnMarketInfoV2, hnMarketV2, getHnImg, erc20, token, contract, getSigner, util } from "hashland-sdk";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    // 去挂单
    goOrder() {
      this.$router.push("/hangingorderV2");
    },
    // 挂单记录
    recordClick() {
